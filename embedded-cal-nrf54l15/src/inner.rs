...
impl Nrf54l15CalInner {
    pub(crate) fn new_inner(cracen: cracen::Cracen, cracen_core: cracencore::Cracencore) -> Self {
...
impl Drop for Nrf54l15CalInner {
    fn drop(&mut self) {
...
impl embedded_cal::plumbing::hash::Sha2Short for Nrf54l15CalInner {
    ...
    fn finalize(&mut self, instance: Self::State, last_chunk: &[u8], target: &mut [u8]) {
        ...
        target.copy_from_slice(&instance.state.expect(
            "Sha2Short::finalize called without any prior update(); \
           call update() with at least one block before finalize()",
        ));
    }
}
