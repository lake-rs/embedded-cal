...
fn sha256(cal: &mut Stm32wba55Cal) {
    ...
    HashProvider::update(cal, &mut hash, &data[..5]);
    HashProvider::update(cal, &mut hash, &data[5..]);
...
fn hmac_sha256(cal: &mut Stm32wba55Cal) {
    let alg = <Stm32wba55Cal as HmacProvider>::Algorithm::from_cose_number(5i8).unwrap();
